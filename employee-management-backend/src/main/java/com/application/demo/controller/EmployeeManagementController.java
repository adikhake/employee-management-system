...
import com.application.demo.dto.UpdateRequestDTO;
...
public class EmployeeManagementController {
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<EmployeeResponse> getMyProfile(@PathVariable Long id) {
        EmployeeResponse resp = employeeService.getMyDetails(id);
        return ResponseEntity.ok(resp);
    }
   
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<EmployeeResponse> update(@PathVariable Long id, @RequestBody UpdateRequestDTO req) {
    ...
    @GetMapping("/me")
    public Employee getMe(@AuthenticationPrincipal UserDetails userDetails) {
